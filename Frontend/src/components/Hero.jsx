...
const Hero = () => {
  ...
  return (
    <main className='w-screen z-30 min-h-[650px] h-screen max-h-[950px] absolute top-0 left-0 pt-[15vh] lg:pt-[21vh] px-5 lg:px-0 flex items-start justify-center'>
      {theme == 'dark'
      ...
      <article className='w-fit h-fit relative z-40 flex flex-col gap-10 items-center justify-start'>
        <Link to={`${currentUser?.userData ? '/dashboard' : '/login'}`}>
          <section className='w-fit bg-transparent rounded-full text-[.6rem] md-mobile:text-sm newFeatureShadow cursor-pointer'>
            <section className='flex gap-2 items-center rounded-full px-4 py-2 shadow-inner dark:shadow-[#8b9da2] shadow-[#fbfbfb] bg-[#1B3334] bg-opacity-0 '>
